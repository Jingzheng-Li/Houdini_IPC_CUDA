...
__global__ void transformLissajous(float3* positions, int numPoints, float time) {
    ...
    positions[idx] = make_float3(newX, newY, newZ);

    // float newX = positions[idx].x + 1.0f;
    // float newY = positions[idx].y + 1.0f;
    // float newZ = positions[idx].z + 1.0f;
    // positions[idx] = make_float3(newX, newY, newZ);
...
void transformLissajousCUDA(float3* cudaPositions, int numPoints, float time) {
    ...
    transformLissajous<<<numBlocks, blockSize>>>(cudaPositions, numPoints, time);
