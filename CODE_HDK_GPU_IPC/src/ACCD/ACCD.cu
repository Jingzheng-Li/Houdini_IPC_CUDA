...
namespace ACCD {
        ...
        double eta, double thickness)
    {
        ...
        double3 mov = MATHUTILS::__s_vec_multiply(MATHUTILS::__add(temp0, temp1), -0.25);

        dea0 = MATHUTILS::__add(dea0, mov);
        dea1 = MATHUTILS::__add(dea1, mov);
        deb0 = MATHUTILS::__add(deb0, mov);
        deb1 = MATHUTILS::__add(deb1, mov);

        double max_disp_mag = sqrt(MATHUTILS::__m_max(MATHUTILS::__squaredNorm(dea0), MATHUTILS::__squaredNorm(dea1))) + sqrt(MATHUTILS::__m_max(MATHUTILS::__squaredNorm(deb0), MATHUTILS::__squaredNorm(deb1)));
        if (max_disp_mag == 0)
            return 1.0;
        ...
        if (dFunc <= 0) {
            double dists0 = MATHUTILS::__squaredNorm(MATHUTILS::__minus(ea0, eb0));
            double dists1 = MATHUTILS::__squaredNorm(MATHUTILS::__minus(ea0, eb1));
            double dists2 = MATHUTILS::__squaredNorm(MATHUTILS::__minus(ea1, eb0));
            double dists3 = MATHUTILS::__squaredNorm(MATHUTILS::__minus(ea1, eb1));

            dist2_cur = MATHUTILS::__m_min(MATHUTILS::__m_min(dists0, dists1), MATHUTILS::__m_min(dists2, dists3));
        ...
        while (true) {
            ...
            ea0 = MATHUTILS::__add(ea0, MATHUTILS::__s_vec_multiply(dea0, toc_lower_bound));
            ea1 = MATHUTILS::__add(ea1, MATHUTILS::__s_vec_multiply(dea1, toc_lower_bound));
            eb0 = MATHUTILS::__add(eb0, MATHUTILS::__s_vec_multiply(deb0, toc_lower_bound));
            eb1 = MATHUTILS::__add(eb1, MATHUTILS::__s_vec_multiply(deb1, toc_lower_bound));
        ...
        double eta, double thickness)
    {
        ...
        double3 temp1 = MATHUTILS::__add(dt2, dp);
        double3 mov = MATHUTILS::__s_vec_multiply(MATHUTILS::__add(temp0, temp1), -0.25);

        dt0 = MATHUTILS::__add(dt0, mov);
        dt1 = MATHUTILS::__add(dt1, mov);
        dt2 = MATHUTILS::__add(dt2, mov);
        dp = MATHUTILS::__add(dp, mov);

        double disp_mag2_vec0 = MATHUTILS::__squaredNorm(dt0);
        double disp_mag2_vec1 = MATHUTILS::__squaredNorm(dt1);
        double disp_mag2_vec2 = MATHUTILS::__squaredNorm(dt2);

        double max_disp_mag = MATHUTILS::__norm(dp) + sqrt(MATHUTILS::__m_max(disp_mag2_vec0, MATHUTILS::__m_max(disp_mag2_vec1, disp_mag2_vec2)));
        if (max_disp_mag == 0)
            return 1.0;
        ...
        while (true) {
            ...
            p = MATHUTILS::__add(p, MATHUTILS::__s_vec_multiply(dp, toc_lower_bound));
            t0 = MATHUTILS::__add(t0, MATHUTILS::__s_vec_multiply(dt0, toc_lower_bound));
            t1 = MATHUTILS::__add(t1, MATHUTILS::__s_vec_multiply(dt1, toc_lower_bound));
            t2 = MATHUTILS::__add(t2, MATHUTILS::__s_vec_multiply(dt2, toc_lower_bound));
    ...
    inline __device__ bool _insideTriangle(double3 a, double3 b, double3 c, double3 p)
    {
        ...
        double3 ba = MATHUTILS::__minus(b, a);
        double3 ca = MATHUTILS::__minus(c, a);

        n = MATHUTILS::__v_vec_cross(ba, ca);//cross(ba, ca);

        da = MATHUTILS::__minus(a, p);
        db = MATHUTILS::__minus(b, p);
        dc = MATHUTILS::__minus(c, p);
        //da = a - p, db = b - p, dc = c - p;
        if ((wa = MATHUTILS::__v_vec_dot(MATHUTILS::__v_vec_cross(db, dc), n)) < 0.0f) return false;
        if ((wb = MATHUTILS::__v_vec_dot(MATHUTILS::__v_vec_cross(dc, da), n)) < 0.0f) return false;
        if ((wc = MATHUTILS::__v_vec_dot(MATHUTILS::__v_vec_cross(da, db), n)) < 0.0f) return false;

        //Compute barycentric coordinates
        double area2 = MATHUTILS::__v_vec_dot(n, n);
        ...
        double& a, double& b, double& c, double& d, const double& thickness)
    {
        ...
        dab =MATHUTILS::__minus(bd, ad), dac =MATHUTILS::__minus(cd , ad), dap =MATHUTILS::__minus(pd , ad);
        oab =MATHUTILS::__minus(b0, a0), oac =MATHUTILS::__minus(c0 , a0), oap =MATHUTILS::__minus(p0 , a0);
        
        dabXdac = MATHUTILS::__v_vec_cross(dab, dac);
        dabXoac = MATHUTILS::__v_vec_cross(dab, oac);
        oabXdac = MATHUTILS::__v_vec_cross(oab, dac);
        oabXoac = MATHUTILS::__v_vec_cross(oab, oac);
        
        a = MATHUTILS::__v_vec_dot(dap, dabXdac);
        b = MATHUTILS::__v_vec_dot(oap, dabXdac) + MATHUTILS::__v_vec_dot(dap,MATHUTILS::__add(dabXoac , oabXdac));
        c = MATHUTILS::__v_vec_dot(dap, oabXoac) + MATHUTILS::__v_vec_dot(oap,MATHUTILS::__add(dabXoac , oabXdac));
        ...
        const double& errorRate, const double& thickness)
    {
        ...
        MATHUTILS::__NewtonSolverForCubicEquation(a, b, c, d, roots, num, errorRate);
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (_insideTriangle(
                MATHUTILS::__add(MATHUTILS::__s_vec_multiply(ad, r), ta0),
                MATHUTILS::__add(MATHUTILS::__s_vec_multiply(bd, r), tb0),
                MATHUTILS::__add(MATHUTILS::__s_vec_multiply(cd, r), tc0),
                MATHUTILS::__add(MATHUTILS::__s_vec_multiply(qd, r), q0))) {
                if (collisionTime > r) {
                    collisionTime = r;
                }
            }
