...
#include "IPC/GIPCPDerivative.cuh"
...
void ImplicitIntegrator::buildFrictionSets() {
    CUDA_SAFE_CALL(cudaMemset(mc_cpNum, 0, 5 * sizeof(uint32_t)));
    int numbers = h_cpNum[0];
    const unsigned int threadNum = 256;
    int blockNum = (numbers + threadNum - 1) / threadNum;
    _calFrictionLastH_DistAndTan <<<blockNum, threadNum>>> (mc_vertexes, mc_collisonPairs, mc_lambda_lastH_scalar, mc_distCoord, mc_tanBasis, mc_collisonPairs_lastH, m_instance->dHat, m_instance->Kappa, mc_cpNum, h_cpNum[0]);
    CUDA_SAFE_CALL(cudaMemcpy(h_cpNum_last, mc_cpNum, 5 * sizeof(uint32_t), cudaMemcpyDeviceToHost));

    numbers = h_gpNum;
    blockNum = (numbers + threadNum - 1) / threadNum;
    _calFrictionLastH_gd << <blockNum, threadNum >> > (mc_vertexes, mc_groundOffset, mc_groundNormal, mc_environment_collisionPair, mc_lambda_lastH_scalar_gd, mc_collisonPairs_lastH_gd, m_instance->dHat, m_instance->Kappa, h_gpNum);
    h_gpNum_last = h_gpNum;
}

void ImplicitIntegrator::GroundCollisionDetect() {
    int numbers = m_surf_vertexNum;
    const unsigned int threadNum = default_threads;
    int blockNum = (numbers + threadNum - 1) / threadNum; //
    _GroundCollisionDetectIPC << <blockNum, threadNum >> > (mc_vertexes, mc_surfVerts, mc_groundOffset, mc_groundNormal, mc_environment_collisionPair, mc_gpNum, m_instance->dHat, numbers);

}

void ImplicitIntegrator::computeSoftConstraintGradientAndHessian(double3* _gradient) {
...
void ImplicitIntegrator::computeGroundGradientAndHessian(double3* _gradient) {
#ifndef USE_FRICTION  
...
void ImplicitIntegrator::computeCloseGroundVal() {
    ...
    const unsigned int threadNum = default_threads;
    int blockNum = (numbers + threadNum - 1) / threadNum; //
    _computeGroundCloseVal << <blockNum, threadNum >> > (mc_vertexes, mc_groundOffset, mc_groundNormal, mc_environment_collisionPair, m_instance->dTol, mc_closeConstraintID, mc_closeConstraintVal, mc_close_gpNum, numbers);

}

bool ImplicitIntegrator::checkCloseGroundVal() {
...
void ImplicitIntegrator::computeGroundGradient(double3* _gradient, double mKappa) {
    int numbers = h_gpNum;
    const unsigned int threadNum = default_threads;
    int blockNum = (numbers + threadNum - 1) / threadNum; //
...
void ImplicitIntegrator::computeSoftConstraintGradient(double3* _gradient) {
    int numbers = m_softConsNum;
...
void ImplicitIntegrator::buildCP() {
...
void ImplicitIntegrator::buildFullCP(const double& alpha) {
...
void ImplicitIntegrator::buildBVH() {
    m_bvh_f->Construct();
    m_bvh_e->Construct();
}

void ImplicitIntegrator::buildBVH_FULLCCD(const double& alpha) {
    m_bvh_f->ConstructFullCCD(mc_moveDir, alpha);
    m_bvh_e->ConstructFullCCD(mc_moveDir, alpha);
}

void ImplicitIntegrator::calBarrierGradientAndHessian(double3* _gradient, double mKappa) {
    int numbers = h_cpNum[0];
    if (numbers < 1)return;
    const unsigned int threadNum = 256;
    int blockNum = (numbers + threadNum - 1) / threadNum;
...
void ImplicitIntegrator::calBarrierHessian() {
...
void ImplicitIntegrator::calFrictionHessian(std::unique_ptr<GeometryManager>& instance) {
...
void ImplicitIntegrator::computeSelfCloseVal() {
    ...
    _calSelfCloseVal << <blockNum, threadNum >> > (mc_vertexes, mc_collisonPairs, mc_closeMConstraintID, mc_closeMConstraintVal,
        mc_close_cpNum, m_instance->dTol, numbers);
}

bool ImplicitIntegrator::checkSelfCloseVal() {
...
void ImplicitIntegrator::calBarrierGradient(double3* _gradient, double mKappa) {
    int numbers = h_cpNum[0];
    if (numbers < 1)return;
    const unsigned int threadNum = 256;
    int blockNum = (numbers + threadNum - 1) / threadNum;

    _calBarrierGradient << <blockNum, threadNum >> > (mc_vertexes, mc_rest_vertexes, mc_collisonPairs, _gradient, m_instance->dHat, mKappa, numbers);

}

void ImplicitIntegrator::calFrictionGradient(double3* _gradient, std::unique_ptr<GeometryManager>& instance) {
    int numbers = h_cpNum_last[0];
    //if (numbers < 1)return;
    const unsigned int threadNum = 256;
    int blockNum = (numbers + threadNum - 1) / threadNum;
...
void ImplicitIntegrator::suggestKappa(double& kappa) {
    double H_b;
...
void ImplicitIntegrator::upperBoundKappa(double& kappa) {
    double H_b;
    compute_H_b(1.0e-16 * m_instance->bboxDiagSize2, m_instance->dHat, H_b);
...
void ImplicitIntegrator::initKappa(std::unique_ptr<GeometryManager>& instance) {
...
double ImplicitIntegrator::Energy_Add_Reduction_Algorithm(int type, std::unique_ptr<GeometryManager>& instance) {
    ...
    case 0:
        ...
        break;
    case 1:
        _getFEMEnergy_Reduction_3D << <blockNum, threadNum, sharedMsize >> > (queue, instance->cudaVertPos, instance->cudaTetElement, instance->cudaDmInverses, instance->cudaTetVolume, numbers, m_instance->lengthRate, m_instance->volumeRate);
        break;
    case 2:
        _getBarrierEnergy_Reduction_3D << <blockNum, threadNum, sharedMsize >> > (queue, instance->cudaVertPos, instance->cudaRestVertPos, mc_collisonPairs, m_instance->Kappa, m_instance->dHat, numbers);
        break;
    case 3:
        _getDeltaEnergy_Reduction << <blockNum, threadNum, sharedMsize >> > (queue, instance->cudaFb, mc_moveDir, numbers);
        break;
    case 4:
        _computeGroundEnergy_Reduction << <blockNum, threadNum, sharedMsize >> > (queue, instance->cudaVertPos, mc_groundOffset, mc_groundNormal, mc_environment_collisionPair, m_instance->dHat, m_instance->Kappa, numbers);
        break;
    case 5:
        ...
        break;
    case 6:
        _getFrictionEnergy_gd_Reduction_3D << <blockNum, threadNum, sharedMsize >> > (queue, instance->cudaVertPos, instance->cudaOriginVertPos, mc_groundNormal, mc_collisonPairs_lastH_gd, numbers, m_instance->IPC_dt, mc_lambda_lastH_scalar_gd, sqrt(m_instance->fDhat) * m_instance->IPC_dt);
        break;
    case 7:
    ...
    case 8:
        ...
        break;
    case 9:
        _computeSoftConstraintEnergy_Reduction << <blockNum, threadNum, sharedMsize >> > (queue, instance->cudaVertPos, instance->cudaTargetVert, instance->cudaTargetIndex, m_instance->softMotionRate, m_animation_fullRate, numbers);
        break;
    case 10:
...
bool ImplicitIntegrator::checkEdgeTriIntersectionIfAny(std::unique_ptr<GeometryManager>& instance) {
...
bool ImplicitIntegrator::checkGroundIntersection() {
    int numbers = h_gpNum;
    const unsigned int threadNum = default_threads;
    int blockNum = (numbers + threadNum - 1) / threadNum; //
...
bool ImplicitIntegrator::isIntersected(std::unique_ptr<GeometryManager>& instance) {
...
void ImplicitIntegrator::updateBoundary(std::unique_ptr<GeometryManager>& instance, double alpha) {
    int numbers = m_vertexNum;
    const unsigned int threadNum = default_threads;
    int blockNum = (numbers + threadNum - 1) / threadNum;//
...
void ImplicitIntegrator::updateBoundaryMoveDir(std::unique_ptr<GeometryManager>& instance, double alpha) {
    int numbers = m_vertexNum;
    const unsigned int threadNum = default_threads;
    int blockNum = (numbers + threadNum - 1) / threadNum;//
    _updateBoundaryMoveDir << <blockNum, threadNum >> > (instance->cudaVertPos, instance->cudaBoundaryType, mc_moveDir, m_instance->IPC_dt, MATHUTILS::PI, alpha, numbers);
}

void ImplicitIntegrator::updateBoundary2(std::unique_ptr<GeometryManager>& instance) {
    int numbers = m_vertexNum;
    const unsigned int threadNum = default_threads;
    int blockNum = (numbers + threadNum - 1) / threadNum;//
...
ImplicitIntegrator::~ImplicitIntegrator() {
    CUDA_FREE_INTEGRATOR();
}

void ImplicitIntegrator::CUDA_FREE_INTEGRATOR() {
    mc_vertexes = nullptr;
