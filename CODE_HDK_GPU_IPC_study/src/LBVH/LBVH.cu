...
__device__ inline 
unsigned int find_split(const uint64_t* node_code, const unsigned int num_leaves, const unsigned int first, const unsigned int last) noexcept {
...
// 1: v0 lies on v2
// 2: v0 lies on v3
// 3: v0 lies on edge v1-v2
// 4: v0 lies on edge v2-v3
// 5: v0 lies on edge v3-v1
// 6: v0 lies inside tri v0-v1-v2
__device__
...
// calculate intersection between edges, if two edges intersect, then exist s and t such that v0+s(u) = v2+t(u)
// 0: two edges intersect on v0
// 1: two edges intersect on v1
// 2: two edges intersect between v0 and v2 (common line)
// 3: two edges intersect on v2
// 4: two edges intersect on v3
// 5: two edges intersect between v1 and v3 (common line)
// 6: two edges intersect between v0 and v1 (common line)
// 7: two edges intersect between v2 and v3 (common line)
// 8: just intersect 
__device__
...
// check whether two edges collision
__device__
inline bool _checkEEintersection(const double3* _vertexes, const double3* _rest_vertexes, const uint32_t& id0, const uint32_t& id1, const uint32_t& id2, const uint32_t& id3, const uint32_t& obj_idx, const double& dHat, uint32_t* _cpNum, int* MatIndex, int4* _collisionPair, int4* _ccd_collisionPair, int edgeNum) noexcept {
	...
	case 0: {
		...
		if (d < dHat) {
			...
			double eeSqureNCross = MATHUTILS::__squaredNorm(MATHUTILS::__v_vec_cross(MATHUTILS::__minus(v0, v1), MATHUTILS::__minus(v2, v3)))/* / MATHUTILS::__squaredNorm(MATHUTILS::__minus(v0, v1))*/; // cross small means nearly parallel
...
// combine AABB together with reduction way
__global__
...
// calculate each triangle BVH, get min/max, each thread calculate one triangle
...
// calculate each triangle BVH of timestep move space
template <class element_type>
__global__
...
// calculate each triangle BVH with Morton Code
__global__
...
// use DFS to check outer triangle collision for vertex face
__global__
void _selfQuery_vf(const int* _btype, const double3* _vertexes, const uint3* _faces, const uint32_t* _surfVerts, const AABB* _boundVolumes, const Node* _nodes, int4* _collisionPair, int4* _ccd_collisionPair, uint32_t* _cpNum, int* MatIndex, double dHat, int number) {
	...
	*stack_ptr++ = 0; // push root into stack and start traverse from root
...
// calculate all the AABB and get max AABB in reduction way
