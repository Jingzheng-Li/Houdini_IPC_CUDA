...
// 不明白 我是不是在这个之前 至少要先保证这个是正确的呢？
// 或者其实先搞出来一个BVH也是没有问题的？
...
void initScene1() {
    ...
    tetMesh.load_tetrahedraMesh(assets_dir + "tetMesh/bunny2.msh", 0.2, make_double3(0, 0.65, 0));
    tetMesh.load_tetrahedraMesh(assets_dir + "tetMesh/bunny2.msh", 0.2, make_double3(0, -0, 0));
	...
	//ipc.USE_MAS = false;
	// ipc.pcg_data.P_type = 1; // use precondtioner
	ipc.pcg_data.P_type = 0; // not use precondtioner
